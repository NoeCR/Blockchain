...
describe('Transaction', () => {
  ...
  it('inputs has a signature the wallet', () => {
  ...
  describe('and updating a transaction', () => {
    ...
    beforeEach(() => {
      nexAmount = 3;
      nextRecipient = 'n3xt-4ddre3ss';
      transaction = transaction.update(wallet, nextRecipient, nexAmount);
    });

    it('subtracts the next amount from the sender wallet', () => {
      const output = transaction.outputs.find(({ address }) => address === wallet.publicKey);
      expect(output.amount).toEqual(wallet.balance - amount - nexAmount);
    });

    it('outputs an amount for the next recipient ', () => {
      const output = transaction.outputs.find(({ address }) => address === nextRecipient);
      expect(output.amount).toEqual(nexAmount);
    });
  });
});
